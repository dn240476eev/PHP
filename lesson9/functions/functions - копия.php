...
function refreshCard($id, $amount) {
    if(isset($_COOKIE['cart'])){
        $cart1 = unserialize($_COOKIE['cart']);
//        echo 'array_replace';
//        $cart = array_replace($cart,  $_POST['cart_item']);
//        print_r(array_replace($cart,  $_POST['cart_item']));
        echo "<br>";
        echo '$cart refreshCard1';
        print_r($cart1);
        $cart1[$id] = $amount;
        echo "<br>";
        echo 'cart[$id] refreshCard';
        echo $cart1[$id];
        echo "<br>";
        echo '$cart refreshCard2';
        print_r($cart1);
    } else {
        $cart1[$id] = $amount;
    }
    setcookie('cart', serialize($cart1),time()+3600*24*30, '/', '', false, true);
    echo "<br>";
    echo 'cookie refreshCard';
